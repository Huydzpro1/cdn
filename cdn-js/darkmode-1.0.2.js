;window.CloudflareApps=window.CloudflareApps||{};CloudflareApps.siteId="0899467bddc7b1871d204571f3cfbc1b";CloudflareApps.installs=CloudflareApps.installs||{};;(function(){'use strict'
...
return false}}());CloudflareApps.installs["KUHaqqdHOfP4"]={appId:"mt9QUlJFKGSg",scope:{}};;CloudflareApps.installs["KUHaqqdHOfP4"].options={"account":{"accountId":"Bno2s5BlLBn8","service":"jsguardian"},"free":{"guardInnerHTML":"clean","guardInput":"clean","guardPrompt":"clean","guardTransport":"https-only","guardWhen":"before","guardXSS":"sanitize"}};;CloudflareApps.installs["WNEJHHOLb0N8"]={appId:"0VJ1mCYqPTh4",scope:{}};;CloudflareApps.installs["WNEJHHOLb0N8"].options={"color":{"custom":"#00ff00","strategy":"custom"},"position":{"value":"bottom-right"},"shape":{"icon":"fancy","radius":1,"showBackground":true}};;CloudflareApps.installs["ZQ4yv3deIzLg"]={appId:"fNyZF92yN7oV",scope:{}};;CloudflareApps.installs["ZQ4yv3deIzLg"].options={"EnableDarkMode":"true","positionWidget":"cornerRightBottom","scaleTransform":1.1,"showAdvanced":true};;if(CloudflareApps.matchPage(CloudflareApps.installs['KUHaqqdHOfP4'].URLPatterns)){(function(){"use strict";function cloneObject(obj,done=new Map()){if(done.has(obj)){return done.get(obj);}
...
return"";}
var _addEventListener=window.addEventListener,_setAttribute=Element.prototype.setAttribute,options=cloneObject(typeof(CloudflareApps.installs['KUHaqqdHOfP4'].options)!=="undefined"?CloudflareApps.installs['KUHaqqdHOfP4'].options:{"free":{"guardWhen":"before","guardInnerHTML":"clean","guardPrompt":"clean","guardInput":"clean","guardTransport":"https-only","guardXSS":"sanitize"},"plan":"free"});function isExecutable(str){var functionExp=/([\s\(]*.\(\s*.*\s*\)\s*.*\s*=>)|([\s*.*\s*=>\s*.\{)|([\s\(]*.[Ff]unction\s*.*\s*\(\s*.*\s*\)\s*.*\s*\{)/,templateExp=/(\`(\s*.*)\$\{(\s*.*\s*.*)\}(\s*.*)\`)/;return functionExp.test(str)||templateExp.test(str)};function protect(){if(options.guardInput&&options.guardInput!=="off"){(function(){if(_addEventListener){_addEventListener.call(window,"change",function(event){var target=event.currentTarget;if(target instanceof HTMLInputElement||target instanceof HTMLTextAreaElement){var oldvalue=target.getAttribute("value"),clean=sanitize(target.value);if(oldvalue===clean){return;}
...
if(options.guardWhen==="after"&&_addEventListener){_addEventListener.call(window,"load",protect);}else{protect();}})();}(function(){var script = document.createElement('script');script.src = '/cdn-cgi/apps/body/-jBZsK-QmCDNmEWxHS7V9mOHuTk.js';document.head.appendChild(script);})();