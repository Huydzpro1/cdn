...
fx.messageBox = function (title, message, buttons, callback, useOverlay) {
        ...
        if (buttons instanceof Array) {
            ...
            for (var i = 0; i < buttons.length; i++) {
                ...
                if (typeof button.text == 'string') {
                    ...
                    if (typeof button.href != 'string') button.href = 'javascript:void(0);';
    ...
    for (var i = 0; i < _buttons.length; i++) {
        ...
        jQuery('#button-' + i).click(function () {
            ...
            if (typeof _buttons[order].callback == 'function') _buttons[order].callback();
            jQuery('#' + htmlId).fadeOut('fast', function () {
                jQuery(this).remove();
            });
...
fx.watchingMessbox = function (title, message, buttons, callback, useOverlay) {
    ...
    var buttonHtml = '';
    for (var i = 0; i < _buttons.length; i++) {
        buttonHtml += '<button id="' + _buttons[i].id + '" class="' + _buttons[i].selector + '" title="' + _buttons[i].title + '">' + _buttons[i].text + '</button>';
    }
